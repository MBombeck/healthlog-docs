...
That is harmless: HealthLog recognises the Paperless document id and answers
that it already has it, and a document you deleted in HealthLog is not brought
back.
...
- **A file you had already uploaded by hand** is recognised by its content:
  ...
  for the content to be compared, this first match counts as one upload.
...
upload, within your daily AI budget. The Paperless workflow does not hold reading back unless you add
the `aiRead` parameter.

## Limits and storage

- **Uploads per hour.** A document token may store 120 documents an hour by
  default, so an archive of a thousand documents takes a night. The limit is
  per token, and uploads you make yourself in the web app or on your phone are
  counted separately and are not held up. The operator can change it with
  [`DOCUMENT_UPLOAD_LIMIT_PER_HOUR`](/reference/environment-variables/#documents-optional).
- **Lookups.** Recognising documents HealthLog already has by their source id
  is limited to 5,000 an hour per token, which covers a re-run over a few
  thousand documents. Past that, the script sends the document as an upload
  instead, and HealthLog still recognises it.
- **Waiting.** When an upload is answered with `429`, the script waits for as
  long as the `Retry-After` header says (at most an hour per wait) and carries
  on, as often as it takes. A server error or a dropped connection is retried
  three times, after 5, 15 and 45 seconds.
...
| `… HealthLog asked to slow down, waiting … s`                                           | Not an error: the hourly upload limit is reached (`429`). Let it run; the operator can raise `DOCUMENT_UPLOAD_LIMIT_PER_HOUR`.                                                           |
...
| `… could not connect (…)`                                                               | The address is unreachable from where the script runs; it tried three times first.                                                                                                     |
...
the query string. In the query string HealthLog answers a known key before the
file is read at all.
