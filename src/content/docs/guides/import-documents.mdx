...
- **Lookups.** The script asks before each download whether HealthLog already
  ...
  When it is spent, the script waits until it frees up, as it does for the
  upload limit.
...
the query string, or both, as the script sends it; if both are sent they must
agree. In the query string HealthLog answers a known key before the file is
