...
- **Lookups.** The script asks before each download whether HealthLog already
  ...
  per token, which covers a re-run over a few thousand documents; the check
  HealthLog makes on the key sent with an upload draws on the same allowance.
  When it is spent, the script stops asking and uploads the document, which
  then counts as one upload.
...
| `… HealthLog asked to slow down, waiting … s`                                           | Not an error: the hourly upload limit or the lookup allowance is reached (`429`). Let it run; the operator can raise `DOCUMENT_UPLOAD_LIMIT_PER_HOUR`.                                                           |
...
the query string, or both; if both are sent they must agree. The script sends
it as form fields. In the query string HealthLog answers a known key before the file is
read and without using an upload slot; that check draws on the lookup
allowance.
